...
class QueueReservedJobsCollector implements Collector
{
    ...
    {
        Prometheus::addGauge('Queue reserved jobs')
            ->name('queue_reserved_jobs')
            ->label('connection')
            ->label('queue')
            ->helpText('The number of reserved jobs in the queue')
            ->value(function () {
                ...
                foreach ($this->queues as $queueName) {
                    try {
                        ...
                        if (method_exists($queueConnection, 'reservedSize')) {
                            $reservedSize = $queueConnection->reservedSize($queueName);
                        } else {
                            $reservedSize = 0;
                        }

                        $results[] = [$reservedSize, [$this->connection, $queueName]];
                    } catch (\Exception $e) {
